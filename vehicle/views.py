...
def afterlogin_view(request):
    if is_customer(request.user):
        ...
        accountapproval = models.Mechanic.objects.filter(user=request.user, status=True).exists()
        if accountapproval:
...
def edit_customer_profile_view(request):
    ...
    if request.method == 'POST':
        ...
        if userForm.is_valid() and customerForm.is_valid():
            ...
            # Only set the new password if the user has provided a new one
            new_password = userForm.cleaned_data.get('password')
            if new_password:
                user.set_password(new_password)  # Hash the new password before saving it

            user.save()  # Save the user instance with the updated password (if any)
            customerForm.save()  # Save the customer instance
            return HttpResponseRedirect('customer-profile')  # Redirect to customer profile after saving
...
class CustomerLoginView(LoginView):
    template_name = 'vehicle/customerlogin.html'
    ...
    def form_valid(self, form):
        ...
        if is_customer(user):
            ...
            messages.error(self.request, "Invalid credentials for customer login!")
            return self.form_invalid(form)

class MechanicLoginView(LoginView):
    template_name = 'vehicle/mechaniclogin.html'
    form_class = AuthenticationForm
    ...
    def form_valid(self, form):
        ...
        if is_mechanic(user):
            ...
            messages.error(self.request, "Invalid credentials for mechanic login!")
            return self.form_invalid(form)

class AdminLoginView(LoginView):
    template_name = 'vehicle/adminlogin.html'
    form_class = AuthenticationForm
    ...
    def form_valid(self, form):
        ...
        if is_admin(user):
            ...
            messages.error(self.request, "Invalid credentials for admin login!")
            return self.form_invalid(form)
...
def mechanic_dashboard_view(request):
    mechanic = get_object_or_404(models.Mechanic, user_id=request.user.id)
